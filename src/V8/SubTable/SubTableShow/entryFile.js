...
const CommonRegisterCommand = "DomLoaded.SubTableShow";
...
const LocalFuncToActivate = async () => {
    try {
        ...
        const userInputURL = await vscode.window.showInputBox({
            prompt: 'Enter the endpoint for Show',
        ...
        const configPath = path.join(FinalDestinationPath, "FormLoad", "DOMContentLoaded", "AddListeners", "RefreshBSTableClass", "FetchAsGet","getUrl.json");

        await fse.writeJson(configPath, {
            GetEndPoint: userInputURL,
        });
